...
from optimizers.l1rpopurel1.passmanager import level_3_with_contant_pure
...
class RPOLevel3HoareOptimizer:
    def __init__(self, qasm_file, quantum_backend):
        ...
        self.optimizer_name = 'l1rpopurel1'
    ...
    def optimize_with_save(self, output_file):
        ...
        print("l1RPOPUREL1 1", time.time() - start)
        start = time.time()
        transpiled = self.pm2.run(transpiled)
        print("l1RPOPUREL1 2", time.time() - start)
        start = time.time()
        transpiled = self.pm3.run(transpiled)
        print("l1RPOPUREL1 3", time.time() - start)
