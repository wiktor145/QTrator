...
class FeynmanOptimizer:
    def __init__(self, qasm_file, quantum_backend):
        ...
        # print(qasm_file)
        self.temp_filename = "temp_feynman1.qasm"
        self.optimizer_name = 'l1feynmanl1'

    def optimize_with_save(self, output_file):
        qasm_text = self.load_qasm()
        circuit = get_circuit_from_qasm_string(qasm_text)
        qasm_text = transpile(circuit, optimization_level=1).qasm()
        # print(qasm_text)
        ...
        # print(len(after_transpile_circuit))
    ...
    def transpile_for_backend(self, after_transpile):
        ...
        if self.quantum_backend is not None:
            ...
            circuit = transpile(circuit, backend=self.quantum_backend, optimization_level=1)
            # print(circuit)
        return circuit
